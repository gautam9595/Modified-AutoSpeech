...
def main():
    ...
    cudnn.benchmark = cfg.CUDNN.BENCHMARK              #Enabling cudnn auto-tuner to find the best algorithm to use for the hardware.
    torch.backends.cudnn.deterministic = cfg.CUDNN.DETERMINISTIC                 #Enabling Deterministic Training Process
    torch.backends.cudnn.enabled = cfg.CUDNN.ENABLED                                            #Enabling cudnn
    ...
    # model and optimizer
    model = eval('resnet.{}(num_classes={})'.format(cfg.MODEL.NAME, cfg.MODEL.NUM_CLASSES))                   #Loading the required ResNet Model
    model = model.cuda()            
    ...
    test_loader_identification = torch.utils.data.DataLoader(
        ...
        batch_size=1,
        num_workers=cfg.DATASET.NUM_WORKERS,
        pin_memory=True,                 # To speed-up the computation by loading data in Page-locked memory
        shuffle=True,
        drop_last=True,			# To Drop the last batch if the size of the last batch is not of given size.
    )
    ...
    for epoch in tqdm(range(begin_epoch, cfg.TRAIN.END_EPOCH), desc='train progress'):
        model.train()            #Changing to train mode of the model.
        train_from_scratch(cfg, model, optimizer, train_loader, criterion, epoch, writer_dict, lr_scheduler)
        if epoch % cfg.VAL_FREQ == 0:
            acc = validate_identification(cfg, model, test_loader_identification, criterion)
