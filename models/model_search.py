...
class Cell(nn.Module):
    ...
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
        ...
        if reduction_prev:
            self.preprocess0 = FactorizedReduce(C_prev_prev, C, affine=False).cuda()
        else:
            self.preprocess0 = ReLUConvBN(C_prev_prev, C, 1, 1, 0, affine=False).cuda()
...
class Network(nn.Module):
    ...
    def _initialize_alphas(self):
        ...
        self.alphas_normal = nn.Parameter(1e-3 * torch.randn(k, num_ops, requires_grad=True, device="cuda")).cuda()
        self.alphas_reduce = nn.Parameter(1e-3 * torch.randn(k, num_ops, requires_grad=True, device="cuda")).cuda()
