...
def preprocess_voxceleb1(dataset_root: Path, parition: str, out_dir: Path, skip_existing=False):
    ...
    keep_speaker_ids = [speaker_id for speaker_id, nationality in nationalities.items() if                  #keeping only those speakers who are from above mentioned countries
                        nationality.lower() in anglophone_nationalites]
    print("VoxCeleb1: using samples from %d (presumed anglophone) speakers out of %d." %
          (len(keep_speaker_ids), len(nationalities)))

    # Get the speaker directories for anglophone speakers only
    speaker_dirs = dataset_root.joinpath(parition, 'wav').glob("*")             #(* i.e., All) directories and files in the folder (It doesn't list out the directories and files inside the directory of that folder). In our case there will be only folders only
